...
Manages session history file, intake, informed consent, dispatch to voice-trainer agent. Post-session: writes
session note, compresses file. Agent (voice feminization coaching for trans women and transfeminine people)
owns coaching methodology — this skill manages files and frames.

## When to Use

- User wants voice training or voice feminization coaching — pitch, resonance, vocal weight, intonation,
  articulation, or any other voice dimension.
- User wants to work toward a voice that feels like their own, with their own goals (not necessarily
  "passing").
- User wants to continue previous voice training thread (provide history file path).
- User starting voice training first time (no history file yet).

**User-invocable only.** Voice-trainer agent does not suggest or trigger. Client chooses when to start.

## Procedure

### Knowledge Base Bootstrap (First Use Per System)

Voice-trainer agent requires local knowledge base in `med-db/` (gitignored) — specifically the
`voice-feminization` topic with the core evidence archive. Follow `med-db` skill for bootstrap. Run
Mandatory Pre-Work Bootstrap Check from `.claude/agents/references/voice-training-knowledge-base.md`.
Client prefers to proceed without bootstrapping → note limitation — technique guidance uses training
knowledge, not the archived evidence base.
...
Check if history file exists at `sessions/voice/<client-slug>.md`:

- **Exists** → returning client. Load file. Proceed to Step 0b.
- **Does not exist** → new client. Create `sessions/voice/<client-slug>.md` with initial YAML frontmatter
...
# <Name> — Voice Training History

## Permanent Client Profile

> **This section is never compressed.** It stores intake information and training-essential facts
...
- **Phoniatric / HNO check:** (to be collected)
- **Voice surgery:** (to be collected)
- **GAHT status:** (to be collected)
...
Slug derived from filename (`sessions/voice/<slug>.md` → slug).
...
Read `.claude/skills/start-voice-training/references/knowledge-gap-analysis.md`. Execute full procedure:
scan session history for knowledge gaps (techniques, surgery types, anatomy, measurement tools,
voice-affecting conditions, singing), check coverage across med-db/ and resource files, dispatch
...
Read `.claude/skills/start-voice-training/references/intake-questions.md` for exact question wording, delivery rules,
affirming stance for gender/pronouns. Questions: 1a name confirmation, 1b gender and pronouns,
1c age, 1d phoniatric/HNO check, 1e voice complaints and dysphonia, 1f voice surgery history,
1g GAHT status, 1h voice goals, 1i daily voice demands, 1j measurement access, 1k singing,
1l psychological context, 1m anything else before starting. Each question has own wait point —
Never batch them. Every question can be declined.

After client answers 1m (or declines), write completed Permanent Client Profile to history file
immediately (before proceeding to informed consent). Do not write profile until client finished answering.

**Returning client (history file exists):** Skip all intake questions. Instead: "Welcome back. When we last worked
together on [date], you were [brief thread from last session]. Would you like to pick up from there, or is there
something else you'd like to work on today?"
...
Read `.claude/skills/start-voice-training/references/informed-consent.md` (scripts from voice-trainer agent's
...
Seven non-negotiable elements: (1) AI disclosure, (2) AI cannot hear the client, (3) scope of
relationship, (4) vocal safety limits, (5) crisis redirection, (6) data practices,
(7) client's consent to proceed.
...
Read `.claude/skills/start-voice-training/references/handoff-prompt.md` for dispatch template. Assemble prompt
with client's details from Steps 0–3, then dispatch `voice-trainer` agent.

Agent produces opening coaching message (first turn only — stops where client should respond).
Display agent's opening **verbatim, without framing commentary.** Do not add stage directions, emotional
descriptions, or narration implying client's internal state or reaction (e.g., no "*The trainer smiles
...
After displaying agent's opening, **continue coaching dialogue in this conversation**
by adopting voice-trainer agent's persona and methodology (see `.claude/agents/voice-trainer.md`).
Maintain coaching frame, voice, evidence grounding throughout. Agent's opening is start —
...
(e.g., "I'd like to end here," "that's all for today," "Good session!," natural conversational close),
follow the two-step session ending protocol from the voice-trainer agent:
...
1. Coaching closing message (warm, brief, no new material)
2. Exact string `SESSION_ENDED` on its own line
3. Skill invocation to `end-voice-training` with client slug:

   ```text
   Skill: "end-voice-training", args: "<client-slug>"
   ```
...
together in one response. Persona drops only *after* response is complete — `end-voice-training`
...
If voice-trainer agent (subagent dispatched in Step 4) produced opening turn only and
you carried coaching dialogue since: you are ending the session. Apply
atomic-response rule yourself.

`end-voice-training` skill executes all six steps (trainer self-reflection, session note,
...
`end-voice-training` immediately upon realizing omission. Marker is signal, not
...
handled by `end-voice-training` skill. See `end-voice-training` for
complete procedure. Rule files (`references/session-note-format.md`, `references/compression-rules.md`) remain
authoritative format reference — `end-voice-training` skill reads and applies them.
...
- **Every question to client is wait point.** Procedure instructs you to ask client something →
  ...
  Most frequently violated rule, most damaging when broken.
...
- Structured intake questions (1b–1l) asked **only during first session**. Returning clients skip them.
...
- Crisis screen mandatory. Acute risk → redirection, not coaching.
- Session notes, profile updates, protocol saves, compression, closing statements delegated to
  `end-voice-training` skill after every session. Never perform these steps inline — always invoke skill.
...
- Compression mandatory after every session — handled by `end-voice-training` skill.
- **Permanent Client Profile section never compressed.** Exempt from all compression rules.
- Full session protocols saved to `sessions/voice/protocols/<YYYY>-<MM>-<DD>_S<session-no>_<client-slug>.md` after every
  session — handled by `end-voice-training` skill. NOT auto-read by agent — exist for client reference and explicit lookback only.

- Reference files (`.claude/skills/start-voice-training/references/informed-consent.md`, `handoff-prompt.md`,
...
- Don't duplicate agent's coaching methodology. This skill manages files and frames.
- Match client's language (DE/EN). Use standard German orthography per CLAUDE.md.
- Always write to `sessions/voice/<client-slug>.md` — single file per client, not per session.

## Validation

1. Client identity resolved — language selected (wait point respected), name confirmed (wait point respected),
   slug created, history file at `sessions/voice/<client-slug>.md` located or created.
2. **Returning client: Pre-session knowledge gap analysis completed (Step 0b).** Session log scanned, coverage
   checked against med-db/ and resource files. Gaps dispatched to med-researcher agent.
...
4. **New client: structured intake collected one question at a time** (name confirmation, gender/pronouns, age,
   phoniatric/HNO check, voice complaints, surgery history, GAHT status, goals, daily voice demands,
   measurement access, singing, psychological context, context). Each question had own wait point — no batched
   questions, no fabricated answers. **Returning client:** welcome-back question asked, wait point respected
   before proceeding to informed consent.
5. Permanent Client Profile populated with intake data and written to history file **after 1m answered**
   and before informed consent (new clients).
6. Informed consent delivered — all seven elements covered, client explicitly consented before proceeding.
   Wait point respected (Step 2).
...
10. **`SESSION_ENDED` marker output AND `end-voice-training` skill invoked atomically in same response** (or client end signal received → skill invoked). Post-session documentation delegated — not performed inline.
11. No duplication of agent's coaching methodology in skill's own output.
12. **At no point was client's side of conversation fabricated, implied, or assumed.**
13. Post-session documentation verified by `end-voice-training` skill's own validation checklist.

## Output

- Complete voice training session — dialogue between client and AI voice trainer.
- Post-session: `end-voice-training` skill invoked → session note, profile update, protocol save, compression, closing statement all handled by delegated skill.
